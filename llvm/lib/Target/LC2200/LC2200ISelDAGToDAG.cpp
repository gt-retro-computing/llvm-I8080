...
class LC2200DAGToDAGISel : public SelectionDAGISel {
 ...
 public:
  ...
  StringRef getPassName() const override {
    return "LC2200 DAG->DAG Pattern Instruction Selection";
  }

  bool runOnMachineFunction(MachineFunction &MF) override {
    Subtarget = &MF.getSubtarget<LC2200Subtarget>();
...
//	bool SelectAddr(SDValue Addr, SDValue &Base, SDValue &Offset);
  ...
  #include "LC2200GenDAGISel.inc"
};

static SDNode *selectImm(SelectionDAG *CurDAG, const SDLoc &DL, int64_t Imm,
                         MVT XLenVT) {

  SDValue ZeroReg = CurDAG->getRegister(LC2200::zero, XLenVT);

  if (isInt<20>(Imm)) {
    SDValue SDImm = CurDAG->getTargetConstant(Imm, DL, XLenVT);

    return CurDAG->getMachineNode(LC2200::ADDI, DL, XLenVT, ZeroReg, SDImm);
  }

  llvm_unreachable("cannot select immediate that doesn't fit in imm20");
}

void LC2200DAGToDAGISel::Select(SDNode *Node) {
  ...
  switch (Opcode) {
    case ISD::Constant: {
      ...
      if (VT == XLenVT && ConstNode->isNullValue()) {
        SDValue New = CurDAG->getCopyFromReg(CurDAG->getEntryNode(), SDLoc(Node),LC2200::zero, XLenVT);
    ...
    case ISD::FrameIndex: {
      ...
      ReplaceNode(Node, CurDAG->getMachineNode(LC2200::ADDI, DL, VT, TFI, Imm));
      return;
    }
//    case ISD::SRL: {
...
bool LC2200DAGToDAGISel::SelectAddrFI(SDValue Addr, SDValue &Base) {
