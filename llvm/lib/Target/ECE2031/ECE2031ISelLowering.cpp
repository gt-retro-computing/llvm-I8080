...
#include "ECE2031GenCallingConv.inc"

ECE2031TargetLowering::ECE2031TargetLowering(const ECE2031TargetMachine &TM,
                                           const ECE2031Subtarget &STI)
    : TargetLowering(TM), Subtarget(STI) {

  // Set up the register classes.
  addRegisterClass(MVT::i32, &ECE2031::GRRegsRegClass);
...
void ECE2031TargetLowering::analyzeInputArgs(
  ...
  for (unsigned i = 0; i != NumArgs; ++i) {
    ...
    if (IsRet) {
      ...
      if (RetCC_ECE2031(i, ArgVT, ArgVT, CCValAssign::Full, ArgFlags, CCInfo)) {
      ...
      if (CC_ECE2031(i, ArgVT, ArgVT, CCValAssign::Full, ArgFlags, CCInfo)) {
        LLVM_DEBUG(dbgs() << "InputArg #" << i << " has unhandled type "
                          << EVT(ArgVT).getEVTString() << '\n');
        llvm_unreachable(nullptr);
      }
...
void ECE2031TargetLowering::analyzeOutputArgs(
    MachineFunction &MF, CCState &CCInfo,
...
SDValue ECE2031TargetLowering::LowerFormalArguments(
...
SDValue
ECE2031TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                  bool IsVarArg,
  ...
  return DAG.getNode(ECE2031ISD::RET, DL, MVT::Other, RetOps);
}

SDValue
ECE2031TargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, ECE2031::sp, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(ECE2031ISD::CALL, DL, NodeTys, Ops);
  Glue = Chain.getValue(1);
...
SDValue ECE2031TargetLowering::LowerOperation(SDValue Op,
...
SDValue ECE2031TargetLowering::lowerGlobalAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue ECE2031TargetLowering::ExpandLibCall(const char *LibcallName, SDValue Op, bool isSigned, SelectionDAG &DAG) const {
...
SDValue ECE2031TargetLowering::lowerShiftLeft(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue ECE2031TargetLowering::lowerBr(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DagLoc(Op);
  SDValue Chain = Op.getOperand(0);
  SDValue Dest = Op.getOperand(1);
  SDLoc DL(Op);

  SDValue Jmp = DAG.getNode(ECE2031ISD::JMP, DL, MVT::Other, Chain, Dest);
  return Jmp;
}

SDValue ECE2031TargetLowering::lowerBrCc(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DagLoc(Op);
  SDValue Chain = Op.getOperand(0);
  ...
  SDLoc DL(Op);
  SDValue Cmp = DAG.getNode(ECE2031ISD::CMP_JMP, DL, MVT::Glue, Chain,
                            DAG.getConstant(CC, DL, MVT::i32), LHS, RHS, Dest);

  return Cmp;
}

SDValue ECE2031TargetLowering::lowerSelectCc(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DagLoc(Op);
  ...
  SDValue SelectMove = DAG.getNode(ECE2031ISD::CMP_SELECT_MOVE, DL, TrueType,
...
static SDValue notValue(SelectionDAG &DAG, const SDLoc &DL, EVT VT, SDValue Value) {
  return DAG.getNode(ECE2031ISD::NAND, DL, VT, Value, Value);
}

SDValue ECE2031TargetLowering::lowerAnd(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  EVT VT = LHS.getValueType();
  SDLoc DL(Op);
  // A AND B = (A NAND B) NAND (A NAND B)
  SDValue Nand = DAG.getNode(ECE2031ISD::NAND, DL, VT, LHS, RHS);
  SDValue And = notValue(DAG, DL, VT, Nand);
  return And;
}

SDValue ECE2031TargetLowering::lowerOr(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  EVT VT = LHS.getValueType();
  SDLoc DL(Op);
  ...
  SDValue Or = DAG.getNode(ECE2031ISD::NAND, DL, VT, NotA, NotB);
  return Or;
}

SDValue ECE2031TargetLowering::lowerXor(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  SDValue NandAB = DAG.getNode(ECE2031ISD::NAND, DL, VT, LHS, RHS);

  SDValue NandABNandA = DAG.getNode(ECE2031ISD::NAND, DL, VT, NandAB, LHS);
  SDValue NandABNandB = DAG.getNode(ECE2031ISD::NAND, DL, VT, NandAB, RHS);

  SDValue Xor = DAG.getNode(ECE2031ISD::NAND, DL, VT, NandABNandA, NandABNandB);
  return Xor;
}

const char *ECE2031TargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((ECE2031ISD::NodeType)Opcode) {
  case ECE2031ISD::FIRST_NUMBER:
    break;
  case ECE2031ISD::RET:
    return "ECE2031ISD::RET";
  case ECE2031ISD::CALL:
    return "ECE2031ISD::CALL";
  case ECE2031ISD::JMP:
    return "ECE2031ISD::JMP";
  case ECE2031ISD::NAND:
    return "ECE2031ISD::NAND";
  case ECE2031ISD::CMP_SKIP:
    return "ECE2031ISD::CMP_SKIP";
  case ECE2031ISD::CMP_JMP:
    return "ECE2031ISD::CMP_JMP";
  case ECE2031ISD::SELECT_MOVE:
    return "ECE2031ISD::SELECT_MOVE";
  case ECE2031ISD::CMP_SELECT_MOVE:
    return "ECE2031ISD::CMP_SELECT_MOVE";
  }
  return nullptr;
}
